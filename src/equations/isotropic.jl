...
function _Θ(::Isotropic, x, y, z, A₀, B₀, a, b, c, d,
        Q, 𝑘ₛ, δₛ, hᶜ, Xᶜ, Yᶜ, Θ₁D, number_of_term) #a,b,c,d,Q,𝑘ₛ,Xᶜ,Yᶜ,Φ,Θ₁D,number_of_term
    sum_m = 0.0
    sum_n = 0.0
    sum_mn = 0.0
    @inbounds @fastmath @simd for i in 1:number_of_term
        Aₘ, Bₘ, Aₙ, Bₙ,
        λ, δ=_calc_coefficients_vectorial!(i, a, b, c, d, Q, 𝑘ₛ, δₛ, hᶜ, Xᶜ, Yᶜ)
    ...
    @inbounds @fastmath @simd for i in 1:number_of_term
        @inbounds @fastmath @simd for j in 1:number_of_term
            Aₘₙ, Bₘₙ, λ,
            δ, β=_calc_coefficients_matrix!(i, j, a, b, c, d, Q, 𝑘ₛ, δₛ, hᶜ, Xᶜ, Yᶜ)
...
function _get_Θ(::Isotropic, A₀, B₀, a, b, c, d, Q, 𝑘ₛ, δₛ, hᶜ, Xᶜ, Yᶜ, Θ₁D, number_of_term)
    return (x,
        y,
        z) -> _Θ(Isotropic(), x, y, z, A₀, B₀, a, b, c, d,
        Q, 𝑘ₛ, δₛ, hᶜ, Xᶜ, Yᶜ, Θ₁D, number_of_term)
end
